	...
	return visit(tree, {
		...
		buildSpec() {
			...
			if (this.children.length > 3)
				obj.params = this.visit(this.children[2])
			return obj
		},

		parameterList() {
			const params = this.children.filter(c => [ ',' ].indexOf(c.getText()) == -1)
			return params.map(p => this.visit(p))
		},

		jsonLiteral() {
			const literal = this.children[0]
		...
		jsonArray() {
			...
			return elements.map(e => this.visit(e))
		},

		jsonKeyValue() {
			const obj = { }
			const name = this.visit(this.children[0])
			let value = this.visit(this.children[2])
