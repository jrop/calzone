...
function getBuildConfig(options, file, contents) {
	let buildConfig = getAnnotationInfoFromContents(contents) || options.files[file]
	if (!buildConfig)
...
const getTransformationInfo = co.wrap(function * (options, file, contents) {
	...
	if (buildConfig) {
		for (const buildStep of buildConfig) {
			...
			const nextPipe = fn.apply(tail, buildStep.params)
			...
			tail.on('error', e => errorEmitter.emit('error', e))
		}

		return { errors: errorEmitter, head, tail, status: 'built' }
	} else if (options.includeAll) {
		return { errors: errorEmitter, head, tail: head, status: 'copied' }
	} else {
		// skip file
