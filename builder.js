...
const waitForBuildPipe = co.wrap(function * (options, file, contents) {
	...
	try {
		...
		case 'built':
			console.log(pathInfo.name.cyan, transformerInfo.status.green)
			break
		case 'copied':
			console.log(pathInfo.name.cyan, transformerInfo.status.blue)
			break
		case 'skipped':
			console.log(pathInfo.name.cyan, transformerInfo.status.yellow)
			break
		}
	} catch (e) {
